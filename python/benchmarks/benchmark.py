...
Compares:
1. Raw file reading (without ACTP) — naive baseline, re-reads all files every session
2. ACTP-packaged context (with ACTP) — pack once, reuse across model switches

Model-Switch Scenario
---------------------
A developer switches AI assistants mid-project (e.g. Claude → GPT → Gemini).
- Without ACTP: every new model session requires pasting the entire codebase again.
- With ACTP:    pack once, hand the compact JSON-LD packet to each model.

Usage:
    python benchmark.py --all
    python benchmark.py --sizes 50 100 200 500
    python benchmark.py --model-switch   # focused model-switch scenario
"""
import argparse
import json
import time
...
class ACTPBenchmark:
    ...
    def benchmark_without_actp(self, repo_path: Path, num_queries: int = 5) -> Dict:
        """
        Baseline: Every model switch / session re-reads ALL text files.

        Mirrors what a developer does manually: copy-paste source files into
        each new chat session.  Binary files and excluded dirs are skipped
        (a reasonable developer would not paste them).
        """
        SKIP_DIRS = {'.git', '__pycache__', 'node_modules', 'dist', 'build'}
        BINARY_EXTS = {
            '.png', '.jpg', '.jpeg', '.gif', '.ico', '.svg',
            '.wasm', '.pyc', '.map', '.bin', '.exe', '.dll', '.so',
            '.zip', '.tar', '.gz',
        }

        print(f"\n📊 [Without ACTP] {num_queries} model sessions (re-read every time)")
        ...
        for q in range(num_queries):
            ...
            raw_context = "\n\n".join(all_content)
            tokens = len(raw_context) // 4  # ~4 chars/token approximation
            
            elapsed = time.perf_counter() - start
            total_tokens += tokens
            total_time += elapsed
            
            print(f"   Session {q+1}: {tokens:,} tokens | {elapsed:.3f}s")
    ...
    def benchmark_with_actp(self, repo_path: Path, num_queries: int = 5) -> Dict:
        """
        ACTP model-switch scenario: pack once, reuse across model sessions.

        Each 'session' (query) simulates handing the ACTP packet to a NEW
        model — the packet is loaded and its semantic context reconstructed.
        This mirrors the real-world flow:
            pack → [Model A session] → switch → [Model B loads packet] → ...
        """
        print(f"\n📦 [With ACTP] {num_queries} model sessions (pack once, reuse)")
        ...
        # ── SAVE (simulate writing the transfer artifact) ──────────────────
        ...
        print(f"   📝 Packed in {pack_time:.3f}s | File: {actp_size / 1024:.1f} KB "
              f"| Files captured: {len(packet_dict.get('files', []))}")
        ...
        for q in range(num_queries):
            ...
            # ── PER-SESSION: load ACTP packet and reconstruct semantic context ──
            # This is what each new model (Claude, GPT, Gemini) receives instead
            # of the full codebase dump.
            ...
            context_parts.append(
                ...
                f"Goal: {project.get('goal', 'Unknown')}"
            )
            
            # All decisions (semantic core — the key value of ACTP)
            for decision in cached.get('decisions', []):
                context_parts.append(
                    f"[{decision.get('priority','?')}] "
                    f"Decision {decision.get('id','?')}: {decision.get('content','')}"
                )
            
            # File manifest (names + sizes, not full content — already in packet)
            files = cached.get('files', [])
            manifest = ", ".join(
                f"{f.get('path','?')} ({f.get('size',0)}B)"
                for f in files[:20]  # top-20 to bound context size
            )
            if manifest:
                context_parts.append(f"Files ({len(files)} total): {manifest}")
            
            # Code snippets metadata
            artifacts = cached.get('artifacts', {})
            for snippet in artifacts.get('code_snippets', [])[:5]:
                context_parts.append(
                    f"Snippet [{snippet.get('id')}]: {snippet.get('summary', '')}"
                )
            ...
            print(f"   Session {q+1} (→ new model): {tokens:,} tokens | {elapsed:.3f}s")
        ...
        return {
            ...
            "files_captured": len(packet_dict.get('files', [])),
        }
    ...
    def print_report(self, data: Dict):
        ...
        print("\n" + "="*70)
        print("✨ ACTP BENCHMARK REPORT — Model-Switch Scenario")
        print("="*70)
        print("Scenario: developer switches AI models mid-project.")
        print("  Without ACTP → paste full codebase into every new session.")
        print("  With ACTP    → hand the compact JSON-LD packet to each model.\n")
        ...
        for r in data.get("benchmarks", []):
            ...
            print(f"\n📊 Repo Size: {size} source files")
            print(f"   {'─' * 66}")
            
            print(f"\n   WITHOUT ACTP (re-paste codebase per model session)")
            print(f"      Text files indexed: {wo.get('file_count', 'N/A')}")
            print(f"      Total text size: {wo.get('total_size', 0) / 1024:.1f} KB")
            print(f"      Avg tokens/session: {wo.get('avg_tokens', 0):,}")
            print(f"      Avg time/session: {wo.get('avg_time', 0):.3f}s")
            
            print(f"\n   WITH ACTP (pack once → reuse on every model switch)")
            print(f"      Pack time (one-time): {wa.get('pack_time', 0):.3f}s")
            print(f"      ACTP file size: {wa.get('actp_size', 0) / 1024:.1f} KB")
            print(f"      Files captured: {wa.get('files_captured', 'N/A')}")
            print(f"      Avg tokens/session: {wa.get('avg_tokens', 0):,}")
            print(f"      Avg time/session: {wa.get('avg_time', 0):.3f}s")
            
            print(f"\n   💾 SAVINGS PER MODEL SWITCH")
...
def main():
    parser = argparse.ArgumentParser(
        description="ACTP Benchmark — Model-Switch: compare with vs without ACTP"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Number of model sessions per benchmark (default: 5)'
    )
